...
let i64_roundtrip () =
  ...
  Alcotest.(check int64)
    "i64 > 2^53 round-trips through abstract accessor" big_i64_int
...
let i64_canonicalize () =
  ...
  Alcotest.(check int64)
    "canonicalize I64 preserves > 2^53 value" big_i64_int
...
let () =
  Alcotest.run "ndarray"
    [
      ( "i64-abstraction-lock",
        [
          Alcotest.test_case "roundtrip >2^53" `Quick i64_roundtrip;
          Alcotest.test_case "canonicalize >2^53" `Quick i64_canonicalize;
        ] );
      ( "canonicalize",
